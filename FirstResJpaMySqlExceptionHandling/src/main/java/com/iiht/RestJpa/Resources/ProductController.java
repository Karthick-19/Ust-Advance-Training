...
import com.iiht.RestJpa.Exceptions.NotFoundException;
...
public class ProductController {
	...
	@GetMapping("error/{id}")
	public ResponseEntity<Product> getProductException(@PathVariable Long id) throws NotFoundException{
		Product singleP = productService.getProductById(id);
		if(singleP == null) {
			throw new NotFoundException("No products found bro");		}
		return new ResponseEntity<Product>(singleP,HttpStatus.OK);
		
	}

	@GetMapping("errorCheck/{id}")
	public ResponseEntity<Product> CheckException(@PathVariable Long id) throws NotFoundException,Exception{
		Product singleP = productService.getProductById(id);
		if(singleP == null) {
			throw new NotFoundException("No products found bro");
		}
